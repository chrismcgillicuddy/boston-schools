...
class TownsMap extends Component {
  ...
  draw() {
    const {matchingTownCount, priceRange, data} = this.props;
    // map setup
    ...
    rankedTownCount = data.filter(function(d){
    ...
    svg.append("g")
      ...
      .enter().append("path")
        ...
        .classed(activeTownClass, function(d) {
          ...
          if ((d.properties[townNameColumn] === defaultTown)) {
            updateTownDetails(d);
            return true;
          }
        })
